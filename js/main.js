...
function addToComparison(type, id) {
    ...
    if (type === 'property') {
        if (!data.properties.includes(idStr)) {
            if (data.properties.length >= 3) {
                window.ComparisonManager.showMessage('Можно сравнивать максимум 3 квартиры', 'warning');
                return;
            }
            data.properties.push(idStr);
            window.ComparisonManager.showMessage('Квартира добавлена в сравнение', 'success');
        } else {
            window.ComparisonManager.showMessage('Квартира уже в сравнении', 'info');
        }
    } else if (type === 'complex') {
        if (!data.complexes.includes(idStr)) {
            if (data.complexes.length >= 3) {
                window.ComparisonManager.showMessage('Можно сравнивать максимум 3 ЖК', 'warning');
                return;
            }
            data.complexes.push(idStr);
            window.ComparisonManager.showMessage('ЖК добавлен в сравнение', 'success');
        } else {
            window.ComparisonManager.showMessage('ЖК уже в сравнении', 'info');
        }
    }
