...
# Configure CSRF protection - TEMPORARILY DISABLED for debugging
...
def manager_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        print(f"DEBUG: manager_required - current_user type: {type(current_user)}")
        print(f"DEBUG: manager_required - request.path: {request.path}")
        print(f"DEBUG: manager_required - request.method: {request.method}")
        print(f"DEBUG: manager_required - content_type: {request.content_type}")
        ...
        print(f"DEBUG: manager_required - headers: {safe_headers}")
        ...
        if not current_user.is_authenticated:
            print(f"DEBUG: manager_required - User not authenticated")
            if is_ajax:
        ...
        if not isinstance(current_user, Manager):
            print(f"DEBUG: manager_required - User is not a Manager, type: {type(current_user)}")
            if is_ajax:
                return jsonify({'success': False, 'error': 'Manager access required'}), 403
            return redirect(url_for('manager_login'))
        
        print(f"DEBUG: manager_required - Success! Manager {current_user.email} authenticated")
        return f(*args, **kwargs)
    return decorated_function
...
👀 Ссылка на презентацию: {request.url_root}presentation/view/{presentation.unique_url}
...
def view_presentation(unique_id):
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        return f"Data loading error: {str(e)}", 500
...
def share_presentation(presentation_id):
    ...
    presentation_url = f"{base_url}/presentation/view/{presentation.unique_url}"
...
def property_print_view(presentation_id, property_id):
    """Print-friendly версия объекта из презентации"""
    from models import Collection, CollectionProperty
